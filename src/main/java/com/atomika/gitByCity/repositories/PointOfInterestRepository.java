...
public interface PointOfInterestRepository extends CrudRepository<PointOfInterestEntity, Long> {
    ...
    @Query("SELECT p FROM PointOfInterestEntity p JOIN fetch p.likes c WHERE c.credential.username = :username")
    List<PointOfInterestEntity> findListLikesFromPointByClient(String username);

    @Query("SELECT p FROM PointOfInterestEntity p join FETCH p.client WHERE p.client.credential.username = :username")
